...
上面的解法可以稍微优化一下：如果当前结点不为空，且既不是p也不是q，那么根据上面的分析，p和q的位置就有三种情况，p和q要么分别位于左右子树中，或者同时位于左子树，或者同时位于右子树。这里**需要优化的情况就是当p和q同时为于左子树或右子树中**，**而且返回的结点并不是p或q**（如果是p或q，不能说明p和q都在left或者都在right，因为递归找到p或者q或null才返回），那么返回值就是p和q的最小父结点了，已经求出来了，就不用再对右结点调用递归函数了，同样，对返回的right也做同样的优化处理。
