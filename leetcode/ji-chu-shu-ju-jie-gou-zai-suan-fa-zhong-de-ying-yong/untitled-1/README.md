...
这道题是根据中序和后序来重建二叉树，依然是用递归来做， 后序的顺序的最后一个肯定是根，所以原二叉树的根节点可以知道，题目中给了一个很关键的条件就是树中没有相同元素，有了这个条件就可以在中序遍历中也定位出根节点的位置，并以根节点的位置将中序遍历拆分为左右两个部分，分别对其递归调用原函数。参考了这里的分析（[http://www.cnblogs.com/grandyang/p/4296193.html](http://www.cnblogs.com/grandyang/p/4296193.html)）。
